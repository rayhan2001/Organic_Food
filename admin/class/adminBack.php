<?php 
	...
	class adminBack{
		...
		public function __construct(){
			...
			$this->conn =mysqli_connect('localhost','root','','e-com');
      ...
      function getCatinfo_category($id){
      ...
      function update_category($receive_data){
      	$ctg_name = $receive_data['up_ctg_name'];
      	$ctg_des = $receive_data['up_ctg_des'];
      	$ctg_id = $receive_data['up_ctg_id'];
      ...
      function add_product($data){
        ...
        $pdt_img_tmp_name = $_FILES['pdt_image']['tmp_name'];
        ...
        if ($pdt_ext == 'jpg' or $pdt_ext == 'png' or $pdt_ext== 'jpeg') {
          ...
          $msg ="You file must be a JPG or PNG file!";
          return $msg;
        }
      }
...
function user_register($data){
  ...
  if ($row==1) {
    ...
    if (strlen($user_mobile)< 11 or strlen($user_mobile)> 11) {
       $msg = "Your Mobile Number Must Be 11 Digit!";
       return $msg;
    }else{
...
function user_login($data){
  ...
  if (mysqli_query($this->conn,$query)) {
    ...
    if ($user_info) {
      ...
      $_SESSION['userPass'] = $user_info['user_password'];
